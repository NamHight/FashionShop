...
const Contact = () => {
  ...
  async function formActionContact(data) {
    ...
    await mutationContact.mutateAsync(data);
    setTimeout(() => {
      setMessage(null);
    }, 5000);
    console.log("data: ", data);
    console.log("register: ", contactRegister);
  }
  return (
    <div className="max-w-6xl mx-auto bg-white p-4 font-[sans-serif]">
      <div className="grid md:grid-cols-2 gap-16 items-center  overflow-hidden p-8 shadow-[0_2px_10px_-3px_rgba(6,81,237,0.3)]  before:right-0 before:w-[300px] before:bg-blue-400 before:h-full max-md:before:hidden">
        <div>
          {message && (
            <div
          ...
          <form method="post" onSubmit={handleContact(formActionContact)}>
            <div className="space-y-4 mt-8">
              ...
              <textarea
                id="description"
                name="description"
                {...contactRegister("description")}
                error={contactErrors.description?.message}
              ...
              {contactErrors.description && (
                <p className="text-red-500 text-sm mt-1">
                  {contactErrors.description?.message}
                </p>
              )}
            </div>
