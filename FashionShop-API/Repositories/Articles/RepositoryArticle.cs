...
namespace FashionShop_API.Repositories.Articles
{
    public class RepositoryArticle : RepositoryBase<Article>, IRepositoryArticle
    {
        ...
        public async Task<PagedListAsync<Article>> GetPagingAndSearchAsync(int page, int limit, string searchName, int categoryid, bool trackChanges)
        {
            ...
            {
                var articles = await FindByCondition(item => item.ArticleName.Contains(searchName) && item.Category_Id == categoryid && item.Status == 1,trackChanges).ToListAsync();
                return await PagedListAsync<Article>.ToPagedListAsync(articles.AsQueryable(), page, limit);
            }
            if (!string.IsNullOrEmpty(searchName))
            {
                var articles = await FindByCondition(item => item.ArticleName.Contains(searchName) && item.Status == 1, trackChanges).ToListAsync();
                return await PagedListAsync<Article>.ToPagedListAsync(articles.AsQueryable(), page, limit);
            }
            if (categoryid != 0)
            {
                var articles = await FindByCondition(item => item.Category_Id == categoryid && item.Status == 1, trackChanges).ToListAsync();
                return await PagedListAsync<Article>.ToPagedListAsync(articles.AsQueryable(), page, limit);
            }
            return await PagedListAsync<Article>.ToPagedListAsync(_context.Articles.Where(item => item.Status == 1).AsQueryable(), page, limit);
        }
    }
}
